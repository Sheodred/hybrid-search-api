...
3. **Handoff doc.** Handoffs live in `C:\Users\<user>\.claude\handoff\hybrid-search-api\`, one flat file per handoff named `<YYYY-MM-DD>T<HHMM>-<slug>.md`. The timestamp prefix sorts newest-last within a project's folder.
   - If this session is clearly associated with this repo, take the newest file in `C:\Users\<user>\.claude\handoff\hybrid-search-api\`, read it, and print the full content so both of you have it in view before continuing.
